...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourPuppet : BehaviourBase {
		...
		/// <summary>
		/// When the puppet is on a moving platform, set this value to match the velocity of that platform.
		/// </summary>
        ...
        /// <summary>
        /// Puppets do not get unpinned if they haven't collided with anything on the "Collision Layers" since the last time they got up.
        /// </summary>
		...
		protected override void OnFixedUpdate(float deltaTime) {
			...
			if (state == State.Unpinned) {
				...
				if (unpinnedTimer >= getUpDelay && canGetUp && !getupDisabled) {
					Vector3 hipV = puppetMaster.muscles[0].rigidbody.velocity - platformVelocity;
					if (hipV.sqrMagnitude < maxGetUpVelocity * maxGetUpVelocity)
					{
